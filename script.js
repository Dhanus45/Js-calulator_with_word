...
const dim=document.getElementById('display');
const lat=document.getElementById('num3');
...
function addop(key)
      ...
      form.onsubmit = (event) => {
        ...
        if (+lat.value in ones) {
          converted.innerHTML = ones[+lat.value];
        }
      
        const numArray = lat.value.split("");
        if (numArray.length === 2 && +lat.value > 19) {
